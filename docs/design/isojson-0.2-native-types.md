...
| F2 — `datetime` / `date` / `time` + `NAIVE_UTC`, `UTC_Z`, `OMIT_MICROSECONDS`, `PASSTHROUGH_DATETIME` | CUJ-1, CUJ-3 | DV-1, DV-3…4b, DV-12, DV-15 |
...
| FR-3 | A cache hit is trusted (strong refs, so no address reuse). On a numpy-step miss, if `sys.modules["numpy"]` is not the cached module, the numpy group is re-read | F1, F3 |
...
| E2E-4 regression, one per DV row (`tests/test_divergence.py`) | CUJ-3 | DV-1, DV-3…16 (4a/4b both; DV-4a also × `NAIVE_UTC\|UTC_Z`, where orjson writes `+00:00` but isojson `Z` — the plain `NAIVE_UTC` combination agrees with orjson and lives in E2E-8 only; an invalid `time` offset: a tzinfo returning an int, and 25h; DV-14 with `M8[s]` NaT, out-of-range, `ps` and generic ≥2-D arrays; DV-15 with int, 24h, ±25h and `str` (for `str`, part (a) asserts no exception and output ≠ reference, since orjson's bytes vary per run); DV-16 with `ps`/`fs`/`as` 1-D). Three parts:<br>(a) orjson 3.12.0 fails: its wrong bytes or message; for DV-14, `json.loads` fails on orjson's bytes; for the death rows (DV-4b datetime, DV-8, DV-9 `1969-12`), a child with `returncode < 0` (POSIX; skipped on Windows with that reason);<br>(b) the reference = `expected_text`, or for declined rows the delegation to `default` and the FR-7 message;<br>(c) isojson == (b) |
